...
int
builtin_echo(const int argc, const char** argv);
int
builtin_type(const int argc, const char** argv);
int
builtin_pwd(const int argc, const char** argv);
int
builtin_complete(const int argc, const char** argv);
int
builtin_jobs(const int argc, const char** argv);
int
...
bool
valid_variable_start(const char* name) {
    ...
    if (argc > 1) {
        if (argc == 2 && strlen(argv[1]) > 0 && isdigit(argv[1][0])) {
            ...
            if (tmp < INT_MAX) {
                ...
                fprintf(stderr,
                        "Passed invalid integer %ld to history builtin, expecting a value lower than %d(INT_MAX)\n",
                        tmp, INT_MAX);
                return -1;
            }
        } else if (argc == 3) {
